...
bool TextureShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"../Engine/assets/texture.vs", L"../Engine/assets/texture.ps");
	if (!result)
	{
		return false;
	}
...
bool TextureShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX& worldMatrix, XMMATRIX& viewMatrix, XMMATRIX& projectionMatrix, ID3D11ShaderResourceView* texture)
{
	...
	XMMATRIX worldMat = XMMatrixTranspose(worldMatrix);
	XMMATRIX viewMat = XMMatrixTranspose(viewMatrix);
	XMMATRIX projectionMat = XMMatrixTranspose(projectionMatrix);
	...
	dataPtr->worldMatrix = worldMat;
	dataPtr->viewMatrix = viewMat;
	dataPtr->projectionMatrix = projectionMat;
